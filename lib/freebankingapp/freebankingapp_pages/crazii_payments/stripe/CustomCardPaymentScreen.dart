...
class _CustomCardPaymentScreenState extends State<CustomCardPaymentScreen> {
  ...
  Future<void> _handlePayPress() async {
    ...
    try {
      ...
      if (paymentIntentResult['error'] != null) {
        ...
        if (context.mounted) {
          scaffoldMessenger.showSnackBar(SnackBar(
              content: Text('${'error'.tr}: ${paymentIntentResult['error']}')));
          return;
        }
      }
      ...
      scaffoldMessenger.showSnackBar(SnackBar(content: Text('${'error'.tr}: $e')));
      rethrow;
    }
  }
  ...
  Future<void> confirmIntent(String paymentIntentId) async {
    ...
    if (result['error'] != null && context.mounted) {
      scaffoldMessenger
          .showSnackBar(SnackBar(content: Text('${'error'.tr}: ${result['error']}')));
    } else {
      scaffoldMessenger.showSnackBar(SnackBar(
          content: Text('payment_success_msg'.tr)));
    }
  }
  ...
  Future<Map<String, dynamic>> callNoWebhookPayEndpointIntentId({
    ...
    final url = Uri.parse('$kApiUrl/charge-card-off-session?lang=$langCode');
  ...
  Future<Map<String, dynamic>> callNoWebhookPayEndpointMethodId({
    ...
    final String langCode = LanguageUtils.getLanguageCode();
    final url = Uri.parse('$kApiUrl/pay-without-webhooks?lang=$langCode');
    final response = await http.post(
      url,
      headers: {
        'Content-Type': 'application/json',
